...
/*******************************************************************************
* Function Name: SENSOR_3_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(SENSOR_3_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef SENSOR_3_I2C_ISR_ENTRY_CALLBACK
    SENSOR_3_I2C_ISR_EntryCallback();
#endif /* SENSOR_3_I2C_ISR_ENTRY_CALLBACK */

#if (SENSOR_3_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = SENSOR_3_I2C_ACK_ADDR;
    ...
    if(NULL != SENSOR_3_customIntrHandler)
    {
        SENSOR_3_customIntrHandler();
    }

    if(SENSOR_3_CHECK_INTR_I2C_EC_MASKED(SENSOR_3_INTR_I2C_EC_WAKE_UP))
    {
    ...
    */
    if(SENSOR_3_CHECK_I2C_FSM_MASTER)
    {
        #if(SENSOR_3_I2C_MASTER)
        {
            ...
            */
            if(SENSOR_3_CHECK_INTR_MASTER_MASKED(SENSOR_3_INTR_MASTER_I2C_BUS_ERROR))
            {
                SENSOR_3_mstrStatus |= (uint16) (SENSOR_3_I2C_MSTAT_ERR_XFER |
                                                         SENSOR_3_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = SENSOR_3_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(SENSOR_3_CHECK_INTR_MASTER_MASKED(SENSOR_3_INTR_MASTER_I2C_ARB_LOST))
            {
                SENSOR_3_mstrStatus |= (uint16) (SENSOR_3_I2C_MSTAT_ERR_XFER |
                                                         SENSOR_3_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = SENSOR_3_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(SENSOR_3_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(SENSOR_3_CHECK_I2C_MASTER_CMD(SENSOR_3_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    SENSOR_3_mstrStatus |= (uint16) (SENSOR_3_I2C_MSTAT_ERR_XFER |
                                                             SENSOR_3_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                SENSOR_3_mstrStatus |= (uint16) SENSOR_3_GET_I2C_MSTAT_CMPLT;

                #if(SENSOR_3_I2C_MULTI_MASTER_SLAVE)
                {
                    if(SENSOR_3_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(SENSOR_3_CHECK_I2C_MASTER_CMD(SENSOR_3_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            SENSOR_3_I2C_MASTER_CLEAR_START;

                            endTransfer = SENSOR_3_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!SENSOR_3_CHECK_INTR_SLAVE_MASKED(SENSOR_3_INTR_SLAVE_I2C_BUS_ERROR))
                               && SENSOR_3_CHECK_INTR_MASTER_MASKED(SENSOR_3_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = SENSOR_3_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            SENSOR_3_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            SENSOR_3_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                            ...
                            SENSOR_3_SetRxInterruptMode(SENSOR_3_NO_INTR_SOURCES);
                        ...
                        else
                        {
                            ...
                            SENSOR_3_state = SENSOR_3_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (SENSOR_3_CHECK_I2C_FSM_SLAVE) */
    {
        #if(SENSOR_3_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or SENSOR_3_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(SENSOR_3_CHECK_INTR_SLAVE_MASKED(SENSOR_3_INTR_SLAVE_I2C_BUS_ERROR |
                                                        SENSOR_3_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(SENSOR_3_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    SENSOR_3_slStatus &= (uint8) ~SENSOR_3_I2C_SSTAT_RD_BUSY;
                    SENSOR_3_slStatus |= (uint8) (SENSOR_3_I2C_SSTAT_RD_ERR |
                                                          SENSOR_3_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    SENSOR_3_slStatus &= (uint8) ~SENSOR_3_I2C_SSTAT_WR_BUSY;
                    SENSOR_3_slStatus |= (uint8) (SENSOR_3_I2C_SSTAT_WR_ERR |
                                                          SENSOR_3_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(SENSOR_3_CHECK_I2C_FSM_MASTER)
    {
        #if(SENSOR_3_I2C_MASTER)
        {
            ...
            */
            if(SENSOR_3_CHECK_INTR_MASTER_MASKED(SENSOR_3_INTR_MASTER_I2C_STOP))
            {
            ...
            {
                if(SENSOR_3_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(SENSOR_3_CHECK_INTR_MASTER_MASKED(SENSOR_3_INTR_MASTER_I2C_NACK))
                    {
                        SENSOR_3_ClearMasterInterruptSource(SENSOR_3_INTR_MASTER_I2C_NACK);

                        SENSOR_3_mstrStatus |= (uint16) (SENSOR_3_I2C_MSTAT_ERR_XFER |
                                                                 SENSOR_3_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(SENSOR_3_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != SENSOR_3_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                SENSOR_3_SetTxInterruptMode(SENSOR_3_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(SENSOR_3_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(SENSOR_3_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(SENSOR_3_CHECK_INTR_RX_MASKED(SENSOR_3_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  SENSOR_3_mstrRdBufSize -
                                        (SENSOR_3_mstrRdBufIndex + SENSOR_3_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > SENSOR_3_I2C_FIFO_SIZE)
                            {
                                diffCount = SENSOR_3_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    SENSOR_3_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                SENSOR_3_mstrRdBufPtr[SENSOR_3_mstrRdBufIndex] = (uint8)
                                                                                        SENSOR_3_RX_FIFO_RD_REG;
                                SENSOR_3_mstrRdBufIndex++;
                            }
                        }
                        /* INTR_RX_NOT_EMPTY:
                        ...
                        */
                        else if(SENSOR_3_CHECK_INTR_RX_MASKED(SENSOR_3_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            SENSOR_3_mstrRdBufPtr[SENSOR_3_mstrRdBufIndex] = (uint8) SENSOR_3_RX_FIFO_RD_REG;
                            SENSOR_3_mstrRdBufIndex++;

                            if(SENSOR_3_mstrRdBufIndex < SENSOR_3_mstrRdBufSize)
                            {
                        ...
                        SENSOR_3_ClearRxInterruptSource(SENSOR_3_INTR_RX_ALL);
                    }
                    else /* Writing */
                    {
                        /* INTR_MASTER_I2C_NACK :
                        ...
                        */
                        if(SENSOR_3_CHECK_INTR_MASTER_MASKED(SENSOR_3_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            SENSOR_3_mstrWrBufIndexTmp -= (SENSOR_3_GET_TX_FIFO_ENTRIES +
                                                                   SENSOR_3_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            SENSOR_3_mstrWrBufIndex = SENSOR_3_mstrWrBufIndexTmp;

                            SENSOR_3_mstrStatus |= (uint16) (SENSOR_3_I2C_MSTAT_ERR_XFER |
                                                                     SENSOR_3_I2C_MSTAT_ERR_SHORT_XFER);

                            SENSOR_3_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(SENSOR_3_CHECK_INTR_TX_MASKED(SENSOR_3_INTR_TX_EMPTY))
                        {
                            while(SENSOR_3_I2C_FIFO_SIZE != SENSOR_3_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(SENSOR_3_mstrWrBufIndexTmp < SENSOR_3_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(SENSOR_3_mstrWrBufIndexTmp == (SENSOR_3_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    SENSOR_3_TX_FIFO_WR_REG = (uint32) SENSOR_3_mstrWrBufPtr[SENSOR_3_mstrWrBufIndexTmp];
                                    SENSOR_3_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            SENSOR_3_ClearTxInterruptSource(SENSOR_3_INTR_TX_ALL);
                        #else
                            SENSOR_3_ClearTxInterruptSource(SENSOR_3_INTR_TX_EMPTY);
                        #endif /* (SENSOR_3_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(SENSOR_3_CHECK_INTR_TX_MASKED(SENSOR_3_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            SENSOR_3_mstrWrBufIndex = SENSOR_3_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(SENSOR_3_CHECK_I2C_MODE_NO_STOP(SENSOR_3_mstrControl))
                    {
    ...
    } /* (SENSOR_3_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(SENSOR_3_CHECK_I2C_FSM_SLAVE)
    {
        #if(SENSOR_3_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(SENSOR_3_CHECK_INTR_SLAVE_MASKED(SENSOR_3_INTR_SLAVE_I2C_NACK))
            {
                SENSOR_3_ClearSlaveInterruptSource(SENSOR_3_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (SENSOR_3_GET_TX_FIFO_ENTRIES + SENSOR_3_GET_TX_FIFO_SR_VALID);

                if(SENSOR_3_slOverFlowCount > diffCount) /* Overflow */
                {
                    SENSOR_3_slStatus |= (uint8) SENSOR_3_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    SENSOR_3_slRdBufIndexTmp -= (diffCount - SENSOR_3_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                SENSOR_3_slRdBufIndex = SENSOR_3_slRdBufIndexTmp;
                ...
                SENSOR_3_slStatus &= (uint8) ~SENSOR_3_I2C_SSTAT_RD_BUSY;
                SENSOR_3_slStatus |= (uint8)  SENSOR_3_I2C_SSTAT_RD_CMPLT;
                SENSOR_3_state     =  SENSOR_3_I2C_FSM_IDLE;
            }
            ...
            */
            if(SENSOR_3_CHECK_INTR_SLAVE_MASKED(SENSOR_3_INTR_SLAVE_I2C_WRITE_STOP))
            {
                SENSOR_3_ClearSlaveInterruptSource(SENSOR_3_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (SENSOR_3_I2C_CTRL_REG & SENSOR_3_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != SENSOR_3_GET_RX_FIFO_ENTRIES)
                    {
                        #if(SENSOR_3_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == SENSOR_3_GET_RX_FIFO_ENTRIES) &&
                               (SENSOR_3_CHECK_INTR_SLAVE_MASKED(SENSOR_3_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        SENSOR_3_slWrBufPtr[SENSOR_3_slWrBufIndex] = (uint8) SENSOR_3_RX_FIFO_RD_REG;
                        SENSOR_3_slWrBufIndex++;
                    }

                    SENSOR_3_DISABLE_SLAVE_AUTO_DATA;
                }

                if(SENSOR_3_CHECK_INTR_RX(SENSOR_3_INTR_RX_OVERFLOW))
                {
                    SENSOR_3_slStatus |= (uint8) SENSOR_3_I2C_SSTAT_WR_OVFL;
                }
                ...
                SENSOR_3_slStatus &= (uint8) ~SENSOR_3_I2C_SSTAT_WR_BUSY;
                SENSOR_3_slStatus |= (uint8)  SENSOR_3_I2C_SSTAT_WR_CMPLT;
                SENSOR_3_state     =  SENSOR_3_I2C_FSM_IDLE;
            }
            ...
            */
        ...
        #endif /* (SENSOR_3_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                SENSOR_3_ClearI2CExtClkInterruptSource(SENSOR_3_INTR_I2C_EC_WAKE_UP);

                #if (SENSOR_3_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != SENSOR_3_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = SENSOR_3_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) SENSOR_3_RX_FIFO_RD_REG;
                        response = SENSOR_3_I2C_ACK_ADDR;
                    }
            ...
            #if (SENSOR_3_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == SENSOR_3_I2C_NAK_ADDR)
                {
                #if (!SENSOR_3_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    SENSOR_3_DISABLE_INTR_SLAVE(SENSOR_3_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!SENSOR_3_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    SENSOR_3_ClearSlaveInterruptSource(SENSOR_3_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    SENSOR_3_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (SENSOR_3_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    ...
                    /* TX direction: master reads from slave */
                    {
                        ...
                        SENSOR_3_slRdBufIndexTmp = SENSOR_3_slRdBufIndex;

                        /* Start master reading */
                        SENSOR_3_slStatus |= (uint8) SENSOR_3_I2C_SSTAT_RD_BUSY;
                        SENSOR_3_state     = SENSOR_3_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (SENSOR_3_slWrBufSize - SENSOR_3_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == SENSOR_3_I2C_FIFO_SIZE)
                            {
                                ...
                                SENSOR_3_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                SENSOR_3_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        SENSOR_3_slStatus |= (uint8) SENSOR_3_I2C_SSTAT_WR_BUSY;
                        SENSOR_3_state     = SENSOR_3_I2C_FSM_SL_WR;
                    }
                    ...
                    SENSOR_3_ENABLE_INTR_SLAVE(SENSOR_3_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!SENSOR_3_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    SENSOR_3_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(SENSOR_3_CHECK_INTR_RX_MASKED(SENSOR_3_INTR_RX_FULL))
            {
                ...
                diffCount =  SENSOR_3_slWrBufSize -
                            (SENSOR_3_slWrBufIndex + SENSOR_3_I2C_FIFO_SIZE);

                if(diffCount > SENSOR_3_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = SENSOR_3_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    SENSOR_3_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                SENSOR_3_ClearRxInterruptSource(SENSOR_3_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = SENSOR_3_RX_FIFO_RD_REG;

                if(SENSOR_3_slWrBufIndex < SENSOR_3_slWrBufSize)
                {
                    SENSOR_3_I2C_SLAVE_GENERATE_ACK;

                    /* Put data into component buffer */
                    SENSOR_3_slWrBufPtr[SENSOR_3_slWrBufIndex] = (uint8) diffCount;
                    SENSOR_3_slWrBufIndex++;
                }
            ...
            */
            if(SENSOR_3_CHECK_INTR_TX_MASKED(SENSOR_3_INTR_TX_EMPTY))
            {
                while(SENSOR_3_I2C_FIFO_SIZE != SENSOR_3_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(SENSOR_3_slRdBufIndexTmp < SENSOR_3_slRdBufSize)
                    /* Data from buffer */
                    {
                        SENSOR_3_TX_FIFO_WR_REG = (uint32) SENSOR_3_slRdBufPtr[SENSOR_3_slRdBufIndexTmp];
                        SENSOR_3_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        SENSOR_3_TX_FIFO_WR_REG = SENSOR_3_I2C_SLAVE_OVFL_RETURN;

                        if(SENSOR_3_slOverFlowCount <= SENSOR_3_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            SENSOR_3_slOverFlowCount++;
                        }
                    }
                }
    ...
    */
    ...
    {
        SENSOR_3_CTRL_REG &= (uint32) ~SENSOR_3_CTRL_ENABLED; /* Disable scb IP */

        SENSOR_3_state = SENSOR_3_I2C_FSM_IDLE;

        SENSOR_3_DISABLE_SLAVE_AUTO_DATA;
        SENSOR_3_DISABLE_MASTER_AUTO_DATA;
        ...
        SENSOR_3_ClearTxInterruptSource(SENSOR_3_INTR_RX_ALL);
        SENSOR_3_ClearRxInterruptSource(SENSOR_3_INTR_TX_ALL);
        SENSOR_3_ClearSlaveInterruptSource(SENSOR_3_INTR_SLAVE_ALL);
        SENSOR_3_ClearMasterInterruptSource(SENSOR_3_INTR_MASTER_ALL);
    #endif /* (SENSOR_3_CY_SCBIP_V0) */

        SENSOR_3_CTRL_REG |= (uint32) SENSOR_3_CTRL_ENABLED;  /* Enable scb IP */
    }

#ifdef SENSOR_3_I2C_ISR_EXIT_CALLBACK
    SENSOR_3_I2C_ISR_ExitCallback();
#endif /* SENSOR_3_I2C_ISR_EXIT_CALLBACK */
