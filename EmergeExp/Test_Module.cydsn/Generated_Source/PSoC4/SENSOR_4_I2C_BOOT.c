...
static uint8 SENSOR_4_slReadBuf[SENSOR_4_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 SENSOR_4_slWriteBuf[SENSOR_4_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 SENSOR_4_applyBuffer;
...
static void SENSOR_4_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: SENSOR_4_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  SENSOR_4_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void SENSOR_4_I2CCyBtldrCommStart(void)
{
    SENSOR_4_I2CSlaveInitWriteBuf(SENSOR_4_slWriteBuf, SENSOR_4_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    SENSOR_4_I2CSlaveInitReadBuf (SENSOR_4_slReadBuf, 0u);

    SENSOR_4_SetCustomInterruptHandler(&SENSOR_4_I2CResposeInsert);
    SENSOR_4_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  SENSOR_4_slRdBufSize - used to store slave read buffer size.
*  SENSOR_4_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  SENSOR_4_slWrBufIndex - used to store current index within slave
*  write buffer.
*  SENSOR_4_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
void SENSOR_4_I2CCyBtldrCommReset(void)
{
    /* Make read buffer full */
    SENSOR_4_slRdBufSize = 0u;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  SENSOR_4_slStatus  - used to store current status of I2C slave.
*  SENSOR_4_slWriteBuf - used to store received command.
*  SENSOR_4_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus SENSOR_4_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (SENSOR_4_I2C_SSTAT_WR_CMPLT & SENSOR_4_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) SENSOR_4_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) SENSOR_4_slWriteBuf,
                                              SENSOR_4_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(SENSOR_4_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  SENSOR_4_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus SENSOR_4_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) SENSOR_4_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        SENSOR_4_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CResposeInsert
****************************************************************************//**
*
...
static void SENSOR_4_I2CResposeInsert(void)
{
    if (SENSOR_4_CHECK_INTR_SLAVE_MASKED(SENSOR_4_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (SENSOR_4_CHECK_I2C_STATUS(SENSOR_4_I2C_STATUS_S_READ))
        {
            ...
            if (0u != SENSOR_4_applyBuffer)
            {
                SENSOR_4_slRdBufSize  = SENSOR_4_applyBuffer;
                SENSOR_4_slRdBufIndex = 0u;
                SENSOR_4_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (SENSOR_4_slRdBufIndex != SENSOR_4_slRdBufSize)
            {
                SENSOR_4_slRdBufIndex = SENSOR_4_slRdBufSize;
            }
        }
    }
}
