...
/* Master variables */
volatile uint16 SENSOR_4_mstrStatus;      /* Master Status byte  */
volatile uint8  SENSOR_4_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * SENSOR_4_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  SENSOR_4_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  SENSOR_4_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * SENSOR_4_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  SENSOR_4_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  SENSOR_4_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  SENSOR_4_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!SENSOR_4_CY_SCBIP_V0 && \
    SENSOR_4_I2C_MULTI_MASTER_SLAVE_CONST && SENSOR_4_I2C_WAKE_ENABLE_CONST)
    static void SENSOR_4_I2CMasterDisableEcAm(void);
#endif /* (!SENSOR_4_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterWriteBuf
****************************************************************************//**
*
...
*  Enables the I2C interrupt and clears SENSOR_4_I2C_MSTAT_WR_CMPLT 
*  status.
*
...
*  - SENSOR_4_I2C_MODE_COMPLETE_XFER - Perform complete transfer 
*    from Start to Stop.
*  - SENSOR_4_I2C_MODE_REPEAT_START - Send Repeat Start instead 
*    of Start. A Stop is generated after transfer is completed unless 
*    NO_STOP is specified.
*  - SENSOR_4_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - SENSOR_4_I2C_MSTR_NO_ERROR - Function complete without error. 
*    The master started the transfer.
*  - SENSOR_4_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - SENSOR_4_I2C_MSTR_NOT_READY - Master is not ready for to start
...
*
* \globalvars
*  SENSOR_4_mstrStatus  - used to store current status of I2C Master.
*  SENSOR_4_state       - used to store current state of software FSM.
*  SENSOR_4_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  SENSOR_4_mstrWrBufPtr - used to store pointer to master write buffer.
*  SENSOR_4_mstrWrBufIndex - used to current index within master write
*  buffer.
*  SENSOR_4_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 SENSOR_4_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = SENSOR_4_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(SENSOR_4_CHECK_I2C_FSM_IDLE)
        {
            SENSOR_4_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = SENSOR_4_CHECK_I2C_STATUS(SENSOR_4_I2C_STATUS_BUS_BUSY) ?
                            SENSOR_4_I2C_MSTR_BUS_BUSY : SENSOR_4_I2C_MSTR_NO_ERROR;
        }
        else if(SENSOR_4_CHECK_I2C_FSM_HALT)
        {
            SENSOR_4_mstrStatus &= (uint16) ~SENSOR_4_I2C_MSTAT_XFER_HALT;
                              errStatus  = SENSOR_4_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(SENSOR_4_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!SENSOR_4_CY_SCBIP_V0 && \
        SENSOR_4_I2C_MULTI_MASTER_SLAVE_CONST && SENSOR_4_I2C_WAKE_ENABLE_CONST)
            SENSOR_4_I2CMasterDisableEcAm();
    #endif /* (!SENSOR_4_CY_SCBIP_V0) */

        /* Set up write transaction */
        SENSOR_4_state = SENSOR_4_I2C_FSM_MSTR_WR_ADDR;
        SENSOR_4_mstrWrBufIndexTmp = 0u;
        SENSOR_4_mstrWrBufIndex    = 0u;
        SENSOR_4_mstrWrBufSize     = cnt;
        SENSOR_4_mstrWrBufPtr      = (volatile uint8 *) wrData;
        SENSOR_4_mstrControl       = (uint8) mode;

        slaveAddress = SENSOR_4_GET_I2C_8BIT_ADDRESS(slaveAddress);

        SENSOR_4_mstrStatus &= (uint16) ~SENSOR_4_I2C_MSTAT_WR_CMPLT;
        ...
        if(SENSOR_4_CHECK_I2C_MODE_RESTART(mode))
        {
            SENSOR_4_I2C_MASTER_GENERATE_RESTART;
            SENSOR_4_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            SENSOR_4_TX_FIFO_WR_REG = slaveAddress;
            SENSOR_4_I2C_MASTER_GENERATE_START;
        }
    }

    SENSOR_4_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterReadBuf
****************************************************************************//**
*
*  Automatically reads an entire buffer of data from a slave device.
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears SENSOR_4_I2C_MSTAT_RD_CMPLT 
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See SENSOR_4_I2CMasterWriteBuf() 
*  function for constants.
*
* \return
*  Error status.See SENSOR_4_I2CMasterWriteBuf() 
*  function for constants.
*
* \globalvars
*  SENSOR_4_mstrStatus  - used to store current status of I2C Master.
...
*  SENSOR_4_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  SENSOR_4_mstrRdBufPtr - used to store pointer to master read buffer.
*  SENSOR_4_mstrRdBufIndex - used to current index within master read
*  buffer.
*  SENSOR_4_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
uint32 SENSOR_4_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(SENSOR_4_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        SENSOR_4_state = SENSOR_4_I2C_FSM_MSTR_RD_ADDR;
        SENSOR_4_mstrRdBufIndex = 0u;
        SENSOR_4_mstrRdBufSize  = cnt;
        SENSOR_4_mstrRdBufPtr   = (volatile uint8 *) rdData;
        SENSOR_4_mstrControl    = (uint8) mode;

        slaveAddress = (SENSOR_4_GET_I2C_8BIT_ADDRESS(slaveAddress) | SENSOR_4_I2C_READ_FLAG);

        SENSOR_4_mstrStatus &= (uint16) ~SENSOR_4_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(SENSOR_4_mstrRdBufSize < SENSOR_4_I2C_FIFO_SIZE)
        {
        ...
        else
        {
            ...
            SENSOR_4_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterSendStart
****************************************************************************//**
*
...
*  - SENSOR_4_I2C_WRITE_XFER_MODE - Set write direction for the 
*    following transfer.
*  - SENSOR_4_I2C_READ_XFER_MODE - Set read direction for the 
*    following transfer.
*
* \return
*  Error status.
*   - SENSOR_4_I2C_MSTR_NO_ERROR - Function complete without error.
*   - SENSOR_4_I2C_MSTR_BUS_BUSY - Bus is busy. 
*     Nothing was sent on the bus. The attempt has to be retried.
*   - SENSOR_4_I2C_MSTR_NOT_READY - Master is not ready for to 
*     start transfer.
...
*     Nothing was sent on the bus. The attempt has to be retried.
*   - SENSOR_4_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was 
*     NAKed.
*   - SENSOR_4_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost 
*     arbitration.
*   - SENSOR_4_I2C_MSTR_ERR_BUS_ERR - Error condition: Master 
*     encountered a bus error. Bus error is misplaced start or stop detection.
*   - SENSOR_4_I2C_MSTR_ERR_ABORT_START - Error condition: The start 
...
uint32 SENSOR_4_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(SENSOR_4_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(SENSOR_4_CHECK_I2C_STATUS(SENSOR_4_I2C_STATUS_BUS_BUSY))
        {
            errStatus = SENSOR_4_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            SENSOR_4_DisableInt();  /* Lock from interruption */
            ...
            else /* Read direction */
            {
                SENSOR_4_state = SENSOR_4_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= SENSOR_4_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            SENSOR_4_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            SENSOR_4_ClearMasterInterruptSource(SENSOR_4_INTR_MASTER_ALL);

            SENSOR_4_I2C_MASTER_GENERATE_START;


            while(!SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_ACK      |
                                                      SENSOR_4_INTR_MASTER_I2C_NACK     |
                                                      SENSOR_4_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_ACK))
            {
                errStatus = SENSOR_4_I2C_MSTR_NO_ERROR;
            }
            else if(SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_NACK))
            {
                errStatus = SENSOR_4_I2C_MSTR_ERR_LB_NAK;
            }
            else if(SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_ARB_LOST))
            {
                SENSOR_4_state = SENSOR_4_I2C_FSM_IDLE;
                             errStatus = SENSOR_4_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = SENSOR_4_I2C_RESET_ERROR;
            }
            else /* SENSOR_4_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                SENSOR_4_state = SENSOR_4_I2C_FSM_IDLE;
                             errStatus = SENSOR_4_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = SENSOR_4_I2C_RESET_ERROR;
            }

            SENSOR_4_ClearMasterInterruptSource(SENSOR_4_INTR_MASTER_I2C_ACK      |
                                                        SENSOR_4_INTR_MASTER_I2C_NACK     |
                                                        SENSOR_4_INTR_MASTER_I2C_ARB_LOST |
                                                        SENSOR_4_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterSendRestart
****************************************************************************//**
*
...
*  See SENSOR_4_I2CMasterSendStart() function for constants.
*
* \return
*  Error status. 
*  See SENSOR_4_I2CMasterSendStart() function for constants.
...
uint32 SENSOR_4_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(SENSOR_4_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = SENSOR_4_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(SENSOR_4_CHECK_I2C_MASTER_CMD(SENSOR_4_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterSendStop
****************************************************************************//**
*
...
uint32 SENSOR_4_I2CMasterSendStop(void)
{
    ...
    if(SENSOR_4_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_STOP     |
                                                  SENSOR_4_INTR_MASTER_I2C_ARB_LOST |
                                                  SENSOR_4_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_STOP))
        {
            errStatus = SENSOR_4_I2C_MSTR_NO_ERROR;
        }
        else if(SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* SENSOR_4_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = SENSOR_4_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = SENSOR_4_I2C_RESET_ERROR;
        }
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterWriteByte
****************************************************************************//**
*
...
uint32 SENSOR_4_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(SENSOR_4_CHECK_I2C_MASTER_ACTIVE)
    {
        SENSOR_4_TX_FIFO_WR_REG = theByte;
        ...
        else /* SENSOR_4_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            SENSOR_4_state = SENSOR_4_I2C_FSM_IDLE;
                         errStatus = SENSOR_4_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = SENSOR_4_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterReadByte
****************************************************************************//**
*
...
*  SENSOR_4_I2CMasterSendStop() or 
*  SENSOR_4_I2CMasterSendRestart() function appropriately.
*  This function is blocking. It does not return until a byte is received or 
*  an error occurs.
*
* \param ackNack: Response to received byte.
*  - SENSOR_4_I2C_ACK_DATA - Generates ACK. 
*     The master notifies slave that transfer continues.
*  - SENSOR_4_I2C_NAK_DATA - Prepares to generate NAK.
...
uint32 SENSOR_4_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = SENSOR_4_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(SENSOR_4_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!SENSOR_4_CHECK_INTR_RX(SENSOR_4_INTR_RX_NOT_EMPTY)) &&
              (!SENSOR_4_CHECK_INTR_MASTER(SENSOR_4_INTR_MASTER_I2C_ARB_LOST |
                                                  SENSOR_4_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(SENSOR_4_CHECK_INTR_RX(SENSOR_4_INTR_RX_NOT_EMPTY))
        {
            theByte = SENSOR_4_RX_FIFO_RD_REG;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes that has been transferred with an
*  SENSOR_4_I2CMasterReadBuf() function.
*
* \return
...
*  SENSOR_4_I2C_MSTAT_ERR_ARB_LOST or
*  SENSOR_4_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  SENSOR_4_mstrRdBufIndex - used to current index within master read
*  buffer.
...
{
    return(SENSOR_4_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterGetWriteBufSize
****************************************************************************//**
*
...
{
    return(SENSOR_4_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterClearWriteBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterStatus
****************************************************************************//**
*
...
*  - SENSOR_4_I2C_MSTAT_RD_CMPLT - Read transfer complete.
*    The error condition status bits must be checked to ensure that 
*    read transfer was completed successfully.
*  - SENSOR_4_I2C_MSTAT_WR_CMPLT - Write transfer complete.
*    The error condition status bits must be checked to ensure that write 
*    transfer was completed successfully.
*  - SENSOR_4_I2C_MSTAT_XFER_INP - Transfer in progress.
*  - SENSOR_4_I2C_MSTAT_XFER_HALT - Transfer has been halted. 
*    The I2C bus is waiting for ReStart or Stop condition generation.
*  - SENSOR_4_I2C_MSTAT_ERR_SHORT_XFER - Error condition: Write 
*    transfer completed before all bytes were transferred. The slave NAKed 
*    the byte which was expected to be ACKed.
*  - SENSOR_4_I2C_MSTAT_ERR_ADDR_NAK - Error condition: Slave did 
*    not acknowledge address.
*  - SENSOR_4_I2C_MSTAT_ERR_ARB_LOST - Error condition: Master lost 
*    arbitration during communications with slave.
*  - SENSOR_4_I2C_MSTAT_ERR_BUS_ERROR - Error condition: bus error 
*    occurred during master transfer due to misplaced Start or Stop 
*    condition on the bus.
*  - SENSOR_4_I2C_MSTAT_ERR_ABORT_XFER - Error condition: Slave was 
...
*  - SENSOR_4_I2C_MSTAT_ERR_XFER - Error condition: This is the 
...
uint32 SENSOR_4_I2CMasterStatus(void)
{
    ...
    status = (uint32) SENSOR_4_mstrStatus;

    if (SENSOR_4_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) SENSOR_4_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CMasterClearStatus
****************************************************************************//**
*
*  Clears all status flags and returns the master status.
*
* \return
*  Current status of master. See the SENSOR_4_I2CMasterStatus() 
*  function for constants.
*
* \globalvars
*  SENSOR_4_mstrStatus - used to store current status of I2C Master.
...
uint32 SENSOR_4_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) SENSOR_4_mstrStatus;
    SENSOR_4_mstrStatus = SENSOR_4_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: SENSOR_4_I2CReStartGeneration
****************************************************************************//**
*
...
void SENSOR_4_I2CReStartGeneration(void)
{
    ...
    */
    uint32 status = SENSOR_4_I2C_STATUS_REG;

    while(SENSOR_4_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    SENSOR_4_SET_I2C_SCL_DR(SENSOR_4_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    SENSOR_4_SET_I2C_SCL_HSIOM_SEL(SENSOR_4_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    SENSOR_4_CTRL_REG &= (uint32) ~SENSOR_4_CTRL_ENABLED;

    if(0u != (status & SENSOR_4_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        SENSOR_4_SET_I2C_SCL_DR(SENSOR_4_I2C_SCL_LOW);
        CyDelayUs(SENSOR_4_I2C_TLOW_TIME); /* Count tLOW */

        SENSOR_4_SET_I2C_SCL_DR(SENSOR_4_I2C_SCL_HIGH);
        while(SENSOR_4_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(SENSOR_4_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    SENSOR_4_I2C_MASTER_CMD_REG = SENSOR_4_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    SENSOR_4_CTRL_REG |= (uint32) SENSOR_4_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    SENSOR_4_SET_I2C_SCL_HSIOM_SEL(SENSOR_4_HSIOM_I2C_SEL);
    ...
    cmd  = SENSOR_4_I2C_MASTER_CMD_M_START;
    cmd |= SENSOR_4_CHECK_I2C_STATUS(SENSOR_4_I2C_STATUS_M_READ) ?
                (SENSOR_4_I2C_MASTER_CMD_M_NACK) : (0u);

    SENSOR_4_I2C_MASTER_CMD_REG = cmd;
    ...
    SENSOR_4_I2C_MULTI_MASTER_SLAVE_CONST && SENSOR_4_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: SENSOR_4_I2CMasterDisableEcAm
    ****************************************************************************//**
    *
    ...
    static void SENSOR_4_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (SENSOR_4_CTRL_REG & SENSOR_4_CTRL_EC_AM_MODE))
        {
            ...
            SENSOR_4_Stop();
            SENSOR_4_CTRL_REG &= (uint32) ~SENSOR_4_CTRL_EC_AM_MODE;
            SENSOR_4_Enable();
        }
    }
#endif /* (!SENSOR_4_CY_SCBIP_V0) */
