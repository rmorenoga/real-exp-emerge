...
/* Master variables */
volatile uint16 SENSOR_2_mstrStatus;      /* Master Status byte  */
volatile uint8  SENSOR_2_mstrControl;     /* Master Control byte */

/* Receive buffer variables */
volatile uint8 * SENSOR_2_mstrRdBufPtr;   /* Pointer to Master Read buffer */
volatile uint32  SENSOR_2_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  SENSOR_2_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * SENSOR_2_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  SENSOR_2_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  SENSOR_2_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  SENSOR_2_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!SENSOR_2_CY_SCBIP_V0 && \
    SENSOR_2_I2C_MULTI_MASTER_SLAVE_CONST && SENSOR_2_I2C_WAKE_ENABLE_CONST)
    static void SENSOR_2_I2CMasterDisableEcAm(void);
#endif /* (!SENSOR_2_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterWriteBuf
****************************************************************************//**
*
...
*  Enables the I2C interrupt and clears SENSOR_2_I2C_MSTAT_WR_CMPLT 
*  status.
*
...
*  - SENSOR_2_I2C_MODE_COMPLETE_XFER - Perform complete transfer 
*    from Start to Stop.
*  - SENSOR_2_I2C_MODE_REPEAT_START - Send Repeat Start instead 
*    of Start. A Stop is generated after transfer is completed unless 
*    NO_STOP is specified.
*  - SENSOR_2_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - SENSOR_2_I2C_MSTR_NO_ERROR - Function complete without error. 
*    The master started the transfer.
*  - SENSOR_2_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - SENSOR_2_I2C_MSTR_NOT_READY - Master is not ready for to start
...
*
* \globalvars
*  SENSOR_2_mstrStatus  - used to store current status of I2C Master.
*  SENSOR_2_state       - used to store current state of software FSM.
*  SENSOR_2_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  SENSOR_2_mstrWrBufPtr - used to store pointer to master write buffer.
*  SENSOR_2_mstrWrBufIndex - used to current index within master write
*  buffer.
*  SENSOR_2_mstrWrBufSize - used to store master write buffer size.
*
*******************************************************************************/
uint32 SENSOR_2_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = SENSOR_2_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(SENSOR_2_CHECK_I2C_FSM_IDLE)
        {
            SENSOR_2_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = SENSOR_2_CHECK_I2C_STATUS(SENSOR_2_I2C_STATUS_BUS_BUSY) ?
                            SENSOR_2_I2C_MSTR_BUS_BUSY : SENSOR_2_I2C_MSTR_NO_ERROR;
        }
        else if(SENSOR_2_CHECK_I2C_FSM_HALT)
        {
            SENSOR_2_mstrStatus &= (uint16) ~SENSOR_2_I2C_MSTAT_XFER_HALT;
                              errStatus  = SENSOR_2_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(SENSOR_2_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!SENSOR_2_CY_SCBIP_V0 && \
        SENSOR_2_I2C_MULTI_MASTER_SLAVE_CONST && SENSOR_2_I2C_WAKE_ENABLE_CONST)
            SENSOR_2_I2CMasterDisableEcAm();
    #endif /* (!SENSOR_2_CY_SCBIP_V0) */

        /* Set up write transaction */
        SENSOR_2_state = SENSOR_2_I2C_FSM_MSTR_WR_ADDR;
        SENSOR_2_mstrWrBufIndexTmp = 0u;
        SENSOR_2_mstrWrBufIndex    = 0u;
        SENSOR_2_mstrWrBufSize     = cnt;
        SENSOR_2_mstrWrBufPtr      = (volatile uint8 *) wrData;
        SENSOR_2_mstrControl       = (uint8) mode;

        slaveAddress = SENSOR_2_GET_I2C_8BIT_ADDRESS(slaveAddress);

        SENSOR_2_mstrStatus &= (uint16) ~SENSOR_2_I2C_MSTAT_WR_CMPLT;
        ...
        if(SENSOR_2_CHECK_I2C_MODE_RESTART(mode))
        {
            SENSOR_2_I2C_MASTER_GENERATE_RESTART;
            SENSOR_2_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            SENSOR_2_TX_FIFO_WR_REG = slaveAddress;
            SENSOR_2_I2C_MASTER_GENERATE_START;
        }
    }

    SENSOR_2_EnableInt();   /* Release lock */
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterReadBuf
****************************************************************************//**
*
*  Automatically reads an entire buffer of data from a slave device.
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears SENSOR_2_I2C_MSTAT_RD_CMPLT 
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See SENSOR_2_I2CMasterWriteBuf() 
*  function for constants.
*
* \return
*  Error status.See SENSOR_2_I2CMasterWriteBuf() 
*  function for constants.
*
* \globalvars
*  SENSOR_2_mstrStatus  - used to store current status of I2C Master.
...
*  SENSOR_2_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  SENSOR_2_mstrRdBufPtr - used to store pointer to master read buffer.
*  SENSOR_2_mstrRdBufIndex - used to current index within master read
*  buffer.
*  SENSOR_2_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
uint32 SENSOR_2_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(SENSOR_2_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        SENSOR_2_state = SENSOR_2_I2C_FSM_MSTR_RD_ADDR;
        SENSOR_2_mstrRdBufIndex = 0u;
        SENSOR_2_mstrRdBufSize  = cnt;
        SENSOR_2_mstrRdBufPtr   = (volatile uint8 *) rdData;
        SENSOR_2_mstrControl    = (uint8) mode;

        slaveAddress = (SENSOR_2_GET_I2C_8BIT_ADDRESS(slaveAddress) | SENSOR_2_I2C_READ_FLAG);

        SENSOR_2_mstrStatus &= (uint16) ~SENSOR_2_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(SENSOR_2_mstrRdBufSize < SENSOR_2_I2C_FIFO_SIZE)
        {
        ...
        else
        {
            ...
            SENSOR_2_ENABLE_MASTER_AUTO_DATA_ACK;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterSendStart
****************************************************************************//**
*
...
*  - SENSOR_2_I2C_WRITE_XFER_MODE - Set write direction for the 
*    following transfer.
*  - SENSOR_2_I2C_READ_XFER_MODE - Set read direction for the 
*    following transfer.
*
* \return
*  Error status.
*   - SENSOR_2_I2C_MSTR_NO_ERROR - Function complete without error.
*   - SENSOR_2_I2C_MSTR_BUS_BUSY - Bus is busy. 
*     Nothing was sent on the bus. The attempt has to be retried.
*   - SENSOR_2_I2C_MSTR_NOT_READY - Master is not ready for to 
*     start transfer.
...
*     Nothing was sent on the bus. The attempt has to be retried.
*   - SENSOR_2_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was 
*     NAKed.
*   - SENSOR_2_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost 
*     arbitration.
*   - SENSOR_2_I2C_MSTR_ERR_BUS_ERR - Error condition: Master 
*     encountered a bus error. Bus error is misplaced start or stop detection.
*   - SENSOR_2_I2C_MSTR_ERR_ABORT_START - Error condition: The start 
...
uint32 SENSOR_2_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(SENSOR_2_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(SENSOR_2_CHECK_I2C_STATUS(SENSOR_2_I2C_STATUS_BUS_BUSY))
        {
            errStatus = SENSOR_2_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            SENSOR_2_DisableInt();  /* Lock from interruption */
            ...
            else /* Read direction */
            {
                SENSOR_2_state = SENSOR_2_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= SENSOR_2_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            SENSOR_2_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            SENSOR_2_ClearMasterInterruptSource(SENSOR_2_INTR_MASTER_ALL);

            SENSOR_2_I2C_MASTER_GENERATE_START;


            while(!SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_ACK      |
                                                      SENSOR_2_INTR_MASTER_I2C_NACK     |
                                                      SENSOR_2_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_ACK))
            {
                errStatus = SENSOR_2_I2C_MSTR_NO_ERROR;
            }
            else if(SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_NACK))
            {
                errStatus = SENSOR_2_I2C_MSTR_ERR_LB_NAK;
            }
            else if(SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_ARB_LOST))
            {
                SENSOR_2_state = SENSOR_2_I2C_FSM_IDLE;
                             errStatus = SENSOR_2_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = SENSOR_2_I2C_RESET_ERROR;
            }
            else /* SENSOR_2_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                SENSOR_2_state = SENSOR_2_I2C_FSM_IDLE;
                             errStatus = SENSOR_2_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = SENSOR_2_I2C_RESET_ERROR;
            }

            SENSOR_2_ClearMasterInterruptSource(SENSOR_2_INTR_MASTER_I2C_ACK      |
                                                        SENSOR_2_INTR_MASTER_I2C_NACK     |
                                                        SENSOR_2_INTR_MASTER_I2C_ARB_LOST |
                                                        SENSOR_2_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterSendRestart
****************************************************************************//**
*
...
*  See SENSOR_2_I2CMasterSendStart() function for constants.
*
* \return
*  Error status. 
*  See SENSOR_2_I2CMasterSendStart() function for constants.
...
uint32 SENSOR_2_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(SENSOR_2_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = SENSOR_2_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(SENSOR_2_CHECK_I2C_MASTER_CMD(SENSOR_2_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterSendStop
****************************************************************************//**
*
...
uint32 SENSOR_2_I2CMasterSendStop(void)
{
    ...
    if(SENSOR_2_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        ...
        while(!SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_STOP     |
                                                  SENSOR_2_INTR_MASTER_I2C_ARB_LOST |
                                                  SENSOR_2_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_STOP))
        {
            errStatus = SENSOR_2_I2C_MSTR_NO_ERROR;
        }
        else if(SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* SENSOR_2_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = SENSOR_2_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = SENSOR_2_I2C_RESET_ERROR;
        }
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterWriteByte
****************************************************************************//**
*
...
uint32 SENSOR_2_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(SENSOR_2_CHECK_I2C_MASTER_ACTIVE)
    {
        SENSOR_2_TX_FIFO_WR_REG = theByte;
        ...
        else /* SENSOR_2_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            SENSOR_2_state = SENSOR_2_I2C_FSM_IDLE;
                         errStatus = SENSOR_2_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = SENSOR_2_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterReadByte
****************************************************************************//**
*
...
*  SENSOR_2_I2CMasterSendStop() or 
*  SENSOR_2_I2CMasterSendRestart() function appropriately.
*  This function is blocking. It does not return until a byte is received or 
*  an error occurs.
*
* \param ackNack: Response to received byte.
*  - SENSOR_2_I2C_ACK_DATA - Generates ACK. 
*     The master notifies slave that transfer continues.
*  - SENSOR_2_I2C_NAK_DATA - Prepares to generate NAK.
...
uint32 SENSOR_2_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = SENSOR_2_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(SENSOR_2_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!SENSOR_2_CHECK_INTR_RX(SENSOR_2_INTR_RX_NOT_EMPTY)) &&
              (!SENSOR_2_CHECK_INTR_MASTER(SENSOR_2_INTR_MASTER_I2C_ARB_LOST |
                                                  SENSOR_2_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(SENSOR_2_CHECK_INTR_RX(SENSOR_2_INTR_RX_NOT_EMPTY))
        {
            theByte = SENSOR_2_RX_FIFO_RD_REG;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes that has been transferred with an
*  SENSOR_2_I2CMasterReadBuf() function.
*
* \return
...
*  SENSOR_2_I2C_MSTAT_ERR_ARB_LOST or
*  SENSOR_2_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  SENSOR_2_mstrRdBufIndex - used to current index within master read
*  buffer.
...
{
    return(SENSOR_2_mstrRdBufIndex);
}


/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterGetWriteBufSize
****************************************************************************//**
*
...
{
    return(SENSOR_2_mstrWrBufIndex);
}


/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterClearWriteBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterStatus
****************************************************************************//**
*
...
*  - SENSOR_2_I2C_MSTAT_RD_CMPLT - Read transfer complete.
*    The error condition status bits must be checked to ensure that 
*    read transfer was completed successfully.
*  - SENSOR_2_I2C_MSTAT_WR_CMPLT - Write transfer complete.
*    The error condition status bits must be checked to ensure that write 
*    transfer was completed successfully.
*  - SENSOR_2_I2C_MSTAT_XFER_INP - Transfer in progress.
*  - SENSOR_2_I2C_MSTAT_XFER_HALT - Transfer has been halted. 
*    The I2C bus is waiting for ReStart or Stop condition generation.
*  - SENSOR_2_I2C_MSTAT_ERR_SHORT_XFER - Error condition: Write 
*    transfer completed before all bytes were transferred. The slave NAKed 
*    the byte which was expected to be ACKed.
*  - SENSOR_2_I2C_MSTAT_ERR_ADDR_NAK - Error condition: Slave did 
*    not acknowledge address.
*  - SENSOR_2_I2C_MSTAT_ERR_ARB_LOST - Error condition: Master lost 
*    arbitration during communications with slave.
*  - SENSOR_2_I2C_MSTAT_ERR_BUS_ERROR - Error condition: bus error 
*    occurred during master transfer due to misplaced Start or Stop 
*    condition on the bus.
*  - SENSOR_2_I2C_MSTAT_ERR_ABORT_XFER - Error condition: Slave was 
...
*  - SENSOR_2_I2C_MSTAT_ERR_XFER - Error condition: This is the 
...
uint32 SENSOR_2_I2CMasterStatus(void)
{
    ...
    status = (uint32) SENSOR_2_mstrStatus;

    if (SENSOR_2_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) SENSOR_2_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CMasterClearStatus
****************************************************************************//**
*
*  Clears all status flags and returns the master status.
*
* \return
*  Current status of master. See the SENSOR_2_I2CMasterStatus() 
*  function for constants.
*
* \globalvars
*  SENSOR_2_mstrStatus - used to store current status of I2C Master.
...
uint32 SENSOR_2_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) SENSOR_2_mstrStatus;
    SENSOR_2_mstrStatus = SENSOR_2_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CReStartGeneration
****************************************************************************//**
*
...
*
*******************************************************************************/
void SENSOR_2_I2CReStartGeneration(void)
{
#if(SENSOR_2_CY_SCBIP_V0)
    /* Generates Restart use GPIO and scb IP functionality. Ticket ID#143715,
    * ID#145238 and ID#173656.
    */
    uint32 status = SENSOR_2_I2C_STATUS_REG;

    while(SENSOR_2_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    SENSOR_2_SET_I2C_SCL_DR(SENSOR_2_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    SENSOR_2_SET_I2C_SCL_HSIOM_SEL(SENSOR_2_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    SENSOR_2_CTRL_REG &= (uint32) ~SENSOR_2_CTRL_ENABLED;

    if(0u != (status & SENSOR_2_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        SENSOR_2_SET_I2C_SCL_DR(SENSOR_2_I2C_SCL_LOW);
        CyDelayUs(SENSOR_2_I2C_TLOW_TIME); /* Count tLOW */

        SENSOR_2_SET_I2C_SCL_DR(SENSOR_2_I2C_SCL_HIGH);
        while(SENSOR_2_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(SENSOR_2_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    SENSOR_2_I2C_MASTER_CMD_REG = SENSOR_2_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    SENSOR_2_CTRL_REG |= (uint32) SENSOR_2_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    SENSOR_2_SET_I2C_SCL_HSIOM_SEL(SENSOR_2_HSIOM_I2C_SEL);
    ...
    cmd  = SENSOR_2_I2C_MASTER_CMD_M_START;
    cmd |= SENSOR_2_CHECK_I2C_STATUS(SENSOR_2_I2C_STATUS_M_READ) ?
                (SENSOR_2_I2C_MASTER_CMD_M_NACK) : (0u);

    SENSOR_2_I2C_MASTER_CMD_REG = cmd;
    ...
    SENSOR_2_I2C_MULTI_MASTER_SLAVE_CONST && SENSOR_2_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: SENSOR_2_I2CMasterDisableEcAm
    ****************************************************************************//**
    *
    ...
    static void SENSOR_2_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (SENSOR_2_CTRL_REG & SENSOR_2_CTRL_EC_AM_MODE))
        {
            ...
            SENSOR_2_Stop();
            SENSOR_2_CTRL_REG &= (uint32) ~SENSOR_2_CTRL_EC_AM_MODE;
            SENSOR_2_Enable();
        }
    }
#endif /* (!SENSOR_2_CY_SCBIP_V0) */
