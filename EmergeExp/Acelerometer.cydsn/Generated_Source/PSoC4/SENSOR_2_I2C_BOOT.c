...
static uint8 SENSOR_2_slReadBuf[SENSOR_2_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 SENSOR_2_slWriteBuf[SENSOR_2_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 SENSOR_2_applyBuffer;
...
static void SENSOR_2_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: SENSOR_2_I2CCyBtldrCommStart
****************************************************************************//**
*
...
*
* \globalvars
*  SENSOR_2_applyBuffer - the flag to release the buffer with a response
*  to be read by the host.
*
*******************************************************************************/
void SENSOR_2_I2CCyBtldrCommStart(void)
{
    SENSOR_2_I2CSlaveInitWriteBuf(SENSOR_2_slWriteBuf, SENSOR_2_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    SENSOR_2_I2CSlaveInitReadBuf (SENSOR_2_slReadBuf, 0u);

    SENSOR_2_SetCustomInterruptHandler(&SENSOR_2_I2CResposeInsert);
    SENSOR_2_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CCyBtldrCommReset
****************************************************************************//**
*
...
*
* \globalvars
*  SENSOR_2_slRdBufSize - used to store slave read buffer size.
*  SENSOR_2_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  SENSOR_2_slWrBufIndex - used to store current index within slave
*  write buffer.
*  SENSOR_2_slStatus  - used to store current status of I2C slave.
*
*******************************************************************************/
void SENSOR_2_I2CCyBtldrCommReset(void)
{
    /* Make read buffer full */
    SENSOR_2_slRdBufSize = 0u;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CCyBtldrCommRead
****************************************************************************//**
*
...
* \globalvars
*  SENSOR_2_slStatus  - used to store current status of I2C slave.
*  SENSOR_2_slWriteBuf - used to store received command.
*  SENSOR_2_slWrBufIndex - used to store current index within slave
*  write buffer.
*
*******************************************************************************/
cystatus SENSOR_2_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (SENSOR_2_I2C_SSTAT_WR_CMPLT & SENSOR_2_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) SENSOR_2_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) SENSOR_2_slWriteBuf,
                                              SENSOR_2_BYTES_TO_COPY(*count, size));

                status = CYRET_SUCCESS;
                break;
            }

            CyDelay(SENSOR_2_WAIT_1_MS);
            --timeoutMs;
        }
    }
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CCyBtldrCommWrite
****************************************************************************//**
*
...
*
* \globalvars
*  SENSOR_2_applyBuffer - the flag to release the buffer with
*  to be read by the host.
*
*******************************************************************************/
cystatus SENSOR_2_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) SENSOR_2_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        SENSOR_2_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: SENSOR_2_I2CResposeInsert
****************************************************************************//**
*
...
static void SENSOR_2_I2CResposeInsert(void)
{
    if (SENSOR_2_CHECK_INTR_SLAVE_MASKED(SENSOR_2_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (SENSOR_2_CHECK_I2C_STATUS(SENSOR_2_I2C_STATUS_S_READ))
        {
            ...
            if (0u != SENSOR_2_applyBuffer)
            {
                SENSOR_2_slRdBufSize  = SENSOR_2_applyBuffer;
                SENSOR_2_slRdBufIndex = 0u;
                SENSOR_2_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (SENSOR_2_slRdBufIndex != SENSOR_2_slRdBufSize)
            {
                SENSOR_2_slRdBufIndex = SENSOR_2_slRdBufSize;
            }
        }
    }
}
